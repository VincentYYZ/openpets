...
assert.match(builderConfig, /pet-help-preload\.cjs/);
...
assert.ok(existsSync(join(appDir, "pet-help-preload.cjs")), "pet-help-preload.cjs must exist for pet help error handling.");
...
const petHelpPreloadSource = readFileSync(join(appDir, "pet-help-preload.cjs"), "utf8");
...
assert.match(mainSource, /initializePetMemoryStore\(\)/, "desktop startup must initialize local pet memory before pet help can use it.");
assert.match(loggerSource, /pet\.memory/, "desktop logger must include a pet memory scope.");
assert.match(petMemoryStoreSource, /openpets-memory\.json[\s\S]*?writeMemoryToDisk[\s\S]*?renameSync/, "pet memory must persist to a local JSON file with temp-file rename writes.");
assert.match(petMemoryStoreSource, /recordPetHelpMemory[\s\S]*?maxFacts[\s\S]*?maxConversations/, "pet memory store must bound long-term facts and conversation summaries.");
assert.match(petMemoryContextSource, /buildPetMemoryContext[\s\S]*?markPetMemoryFactsUsedLater[\s\S]*?setTimeout[\s\S]*?markPetMemoryFactsUsed/, "pet memory context must build bounded prompt context and defer usage timestamp writes off the answer path.");
assert.match(petHelpServiceSource, /buildPetMemoryContext[\s\S]*?本地长期记忆/, "pet help prompt must inject local long-term memory context.");
assert.match(petHelpServiceSource, /petHelpChatOnlyTimeoutMs[\s\S]*?shouldUseChatOnlyClaudeMode/, "pet help must classify non-code questions for the faster chat-only path.");
assert.match(petHelpServiceSource, /createClaudePrintArgs[\s\S]*?--no-session-persistence[\s\S]*?--tools/, "pet help chat-only path must disable Claude Code tools and session persistence.");
assert.match(petHelpPreloadSource, /formatPetHelpError[\s\S]*?Error invoking remote method 'openpets:pet-help-ask'/, "pet help UI must strip Electron IPC wrapper text from Claude errors.");
assert.match(petHelpWindowSource, /finishPetHelpRequest\("success"\);\s*recordPetHelpConversationMemoryLater[\s\S]*?function recordPetHelpConversationMemoryLater[\s\S]*?setTimeout[\s\S]*?recordPetHelpConversationMemory/, "pet help window must return successful answers before extracting and writing local memory.");
assert.match(petHelpWindowSource, /recordPetHelpConversationMemory[\s\S]*?summarizePetHelpConversation[\s\S]*?extractPetMemoryFacts[\s\S]*?recordPetHelpMemory/, "pet help window must summarize successful conversations into local memory.");
...
assert.match(traySource, /copy\.openLogsFolder[\s\S]*?openLogsFolder\(/, "desktop tray must expose user-sendable logs for bug reports.");
...
assert.match(petWindowSource, /pickReactionMessage\(display\.reaction, reactionMessageOverrides\)/, "reaction-only bubbles must render randomized messages instead of raw lowercase reaction ids.");
...
assert.match(traySource, /copy\.updateAvailable\(status\.latestVersion \?\? "latest"\)/, "tray menu must surface available updates.");
...
assert.match(windowsSource, /openpets:get-pet-memory[\s\S]*openpets:set-pet-memory-enabled[\s\S]*openpets:clear-pet-memory/, "settings window must expose local pet memory management IPC.");
assert.match(windowsSource, /id="pet-memory-enabled"[\s\S]*id="pet-memory-list"/, "settings UI must include local pet memory controls.");
...
assert.match(windowsSource, /id="pet-walk-speed"[\s\S]*type="range"[\s\S]*normalizePetWalkSpeed/, "settings window must expose and validate a pet walking speed range control.");
...
assert.match(preloadSource, /getPetMemory[\s\S]*renderPetMemorySettings[\s\S]*deletePetMemoryFact/, "settings preload must render and manage local pet memory.");
assert.match(preloadSource, /reaction-animation-table/, "settings preload must render the reaction animation mapping table.");
assert.match(preloadSource, /bindWalkSpeedSlider[\s\S]*petWalkSpeed[\s\S]*Walking speed saved/, "settings preload must bind and persist pet walking speed changes.");
...
assert.match(defaultPetControllerSource, /function getAutoWalkSpeedPx[\s\S]*preferences\.petWalkSpeed/, "default pet auto-walk must read the saved walking speed preference.");
...
assert.match(windowsSource, /function createOnboardingHtml/, "desktop windows must define the onboarding renderer.");
